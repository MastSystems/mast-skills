...
5. **Dependency soundness** — outbound `Depends on` edges that point at retired specs, or implicit dependencies that should be declared. Use `mast describe inbound` and `mast graph --edge deps` to walk. For each outbound `Depends on`, `mast spec read` the upstream spec and verify the version constraint correctness, cited rule existence, and — if the corpus carries a dependency-topology spec (one that restates who may import whom) — that the dependency edge is permitted by it. When a spec changes a closed-set enum (CLI flag values, `lang:` enum, `--edge` enum, `--kind` enum, etc.), check every dependency on the spec that owns the enum is bumped accordingly.
